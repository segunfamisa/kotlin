...
class LexicalChainedScope @JvmOverloads constructor(
    ...
    override fun getContributedClassifierIncludeDeprecated(name: Name, location: LookupLocation): DescriptorWithDeprecation<ClassifierDescriptor>? {
        ...
        val allCandidatesAreDeprecated = memberScopes
            .mapNotNull { it.getContributedClassifierIncludeDeprecated(name, location) }
            .filter { it.descriptor == firstClassifier }
            .all { it.isDeprecated }

        return DescriptorWithDeprecation.create(firstClassifier, allCandidatesAreDeprecated)
    }
