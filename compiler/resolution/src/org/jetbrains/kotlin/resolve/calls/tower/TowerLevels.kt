...
internal open class ScopeBasedTowerLevel protected constructor(
    ...
    override fun getFunctions(
        ...
        resolutionScope.getContributedClassifierIncludeDeprecated(name, location)?.takeIf { it.isDeprecated }?.descriptor?.let {
            getConstructorsOfClassifier(it).mapTo(result) {
                createCandidateDescriptor(
                    it,
                    dispatchReceiver = null,
